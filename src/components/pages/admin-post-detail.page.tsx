                  ...
                  <Tabs defaultValue="Approved" onValueChange={setTabValue}>
                    ...
                    <TabsContent value="Approved">
                      <div className="space-y-4">
                        <p className="text-sm">
                          Duyệt bài đăng này để hiển thị cho tất cả người dùng. Chủ nhà sẽ nhận được thông báo.
                        </p>
                        <Textarea
                    ...
                    <TabsContent value="Rejected">
                      <div className="space-y-4">
                        <p className="text-sm">
                          Từ chối bài đăng này nếu nó vi phạm chính sách của chúng tôi. Chủ nhà sẽ nhận được lý do từ chối.
                        </p>
                        <Textarea
                    ...
                    {history.length > 0 ? (
                      history.map((item, index) => (
                        <div key={item.historyId ?? index} className="border-l-2 border-muted pl-4 py-1">
                          <p className="text-sm font-medium">Bài viết {item.actionType}</p>
                          <p className="text-xs text-muted-foreground">
                            {new Date(item.execAt).toLocaleString()} bởi Admin {item.admin.lastName} {item.admin.firstName}
                          </p>
                          <p className="text-xs">Kết quả: {item.reason}</p>
                        </div>
                      ))
                    ) : (
                      <p className="text-sm text-muted-foreground">Không có lịch sử duyệt bài</p>
                    )}
                  </div>
                </CardContent>
              </Card>
